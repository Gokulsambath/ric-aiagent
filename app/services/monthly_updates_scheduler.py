...
from app.services.monthly_updates_import_service import MonthlyUpdatesImportService
from app.repository.monthly_updates_repo import MonthlyUpdates as MonthlyUpdatesRepo
from app.services.redis_service import RedisService

logger = logging.getLogger(__name__)

class MonthlyUpdatesImportScheduler:
    """Background scheduler for periodic Monthly Updates Excel imports"""
    
    def __init__(self, redis_service: RedisService):
        self.scheduler = BackgroundScheduler()
        self.excel_service = MonthlyUpdatesImportService()
        self.updates_repo = MonthlyUpdatesRepo()
        self.redis_service = redis_service
        self.job_status_key = "monthly_updates_import_job_status"
        
    def start_scheduler(self, interval_minutes: int = 1440):
        """
        Start the background scheduler with specified interval.
        Default: every 24 hours (1440 minutes)
        """
        if self.scheduler.running:
            logger.warning("Monthly Updates scheduler is already running")
            return
        ...
        self.scheduler.add_job(
            ...
            id='monthly_updates_import_job',
            name='Monthly Updates Import Job',
            replace_existing=True
        )
        
        self.scheduler.start()
        logger.info(f"Monthly Updates import scheduler started with {interval_minutes} minute interval")
    
    def stop_scheduler(self):
        """Stop the background scheduler"""
        if self.scheduler.running:
            self.scheduler.shutdown()
            logger.info("Monthly Updates import scheduler stopped")
    
    def run_import_job(self) -> Dict[str, Any]:
        """
        Execute the import job.
        Scans for monthly updates Excel files and imports them.
        """
        ...
        try:
            ...
            self._update_job_status({
                'status': 'running',
                'message': 'Monthly Updates import job started',
                'last_run': job_start.isoformat(),
                'records_processed': 0,
                'records_failed': 0
            })
            
            # Scan for monthly updates Excel files
            excel_files = self.excel_service.scan_monthly_updates_files()
            
            if not excel_files:
                logger.info("No Monthly Updates Excel files found to import")
                self._update_job_status({
                    'status': 'idle',
            ...
            for file_path in excel_files:
                try:
                    ...
                    is_valid, error_msg = self.excel_service.validate_data(df)
                    ...
                    updates_data = self.excel_service.transform_dataframe_to_updates(df)
                    expected_count = len(updates_data)
                    ...
                    if success and expected_count > 0:
                        # Bulk insert
                        inserted_count = self.updates_repo.bulk_insert(updates_data)
            ...
            logger.info(f"Monthly Updates import job completed: {final_status['message']}")
            
            return final_status
            
        except Exception as e:
            error_msg = f"Monthly Updates import job failed: {str(e)}"
    ...
    def trigger_manual_import(self) -> Dict[str, Any]:
        ...
        """
        logger.info("Manual Monthly Updates import triggered")
...
_scheduler_instance: Optional[MonthlyUpdatesImportScheduler] = None

def get_monthly_updates_scheduler(redis_service: RedisService) -> MonthlyUpdatesImportScheduler:
    """Get or create the global Monthly Updates scheduler instance"""
    global _scheduler_instance
    if _scheduler_instance is None:
        _scheduler_instance = MonthlyUpdatesImportScheduler(redis_service)
